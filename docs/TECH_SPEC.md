...
| `src/lib/job-form.ts`, `src/lib/job-constants.ts` | Validation; `JOB_TYPES` (`Install`, `Pickup`, `Drop`, `Other`, `Site Walk`, `Relocate`); `isRelocate`; `allowsEmptyFenceAndMaterials` (Site Walk + Relocate); `allowsZeroHourLabor` (Site Walk only); `JOB_STATUSES` (`Active`, `Cancelled`); `CANCEL_JOB_CONFIRM`; `JOB_CLASSES` vs `SITE_WALK_CLASSES` |
| `src/lib/jobs-list.ts` | Jobs table `where` (date / branch / type / search; optional `?hideCancelled=1` via `parseHideCancelled`); From=To same date is the day list (banner may call out Relocate with no materials) |
...
**Contacts** (ALE-37 / #32): `Job.contact1` and `Job.contact2` are optional nullable text. In `JobFormDetails`, **Revenue**, **Contact 1**, and **Contact 2** share one row (`lg:grid-cols-3`); **Notes** is a full-width textarea below. `validateAndNormalize` trims each contact and stores `null` when the field is blank. Notes are normalized on their own, so clearing contacts does not change notes. `createJob` and `updateJob` persist both columns. Job detail shows them. **Export Project** (`src/lib/export-project.ts`) adds Contact 1 and Contact 2 summary rows. CSV import does not map contact columns (`IMPORT_FIELDS` in `src/lib/csv-import.ts`).
...
**Relocate** (additive, ALE-36 / #31): same class dropdown as Install (`EVENT` / `CONSTRUCTION` / `OTHER` — **not** Non Pay / Site Visit). Create default class is **CONSTRUCTION** (`emptyJobFormValues`; shared with Install / Pickup / Drop / Other). Edit loads the saved class. Fence and materials may be empty via `allowsEmptyFenceAndMaterials` (same placeholder-row skip as Site Walk). **Unlike Site Walk:** `allowsZeroHourLabor` stays Site Walk only — Relocate labor is billable; hours and revenue roll into P&L; a 0/0 labor row is dropped. Inventory sign is 0 (Jobs Inv. = “No inventory effect”). Jobs / From=To same-day list treats Relocate as a normal row (no revenue or materials predicate); the day-list banner may call out Relocate with no materials.

**CSV import** (`src/lib/csv-import.ts`, `/admin/import`) maps known Daily Tracker aliases from `IMPORT_JOB_TYPE_ALIASES` (`INST` → Install, `PU` → Pickup, `DELIVERY` → Drop, `SITEWALK` / `SITE-WALK` → Site Walk, `RELOCATE` / `RELOC` → Relocate; spaces and underscores collapse to `-`, so `SITE WALK` / `site_walk` also map). Unknown codes reject the row (never silent Other), including `SWLK`. There is **no** class alias table — `class` is stored as-is. There is **no** Status column — creates are always `Active`; re-import `jobFields` omit `status` so a cancelled ticket stays cancelled. Import then runs the same `validateAndNormalize` as the form and commits accepted rows in one transaction. Preview shape: [IMPORT.md](./IMPORT.md).
...
Analytics (`src/app/analytics/`, `src/lib/analytics.ts`) is monthly LF by branch × job-type group from **job tickets only**. Groups follow inventory sign: Install / Drop, Pickup, Other (`Site Walk` and `Relocate` group with Other). Cancelled tickets are skipped (`isCancelledStatus`).
...
| **Gates / manual terminals** | Each section | Auto terminals = that section’s gate qty × 2. 6′ slides add ALE-30 extra track-support posts — recipe in [BOM_APPROVED.md](./BOM_APPROVED.md) §7. The terminal post kit (posts, tension bars, bands) emits only when fence LF > 0. Gate-only (LF empty, fence type blank) does not emit it. Chainlink LF 100 with one `24x6 SLIDE` is 7 terminals, not 14. |
| **Screen SKU** | Any | SKU pick (`BLACK6`, …), not Yes/No. `CUSTOM` is a known catalog gap. |
| **Gates** | Any | Gate + Gate 2 type/qty. 6′ slide hardware always emits: carrier, rollers, track brackets, top rail, and the leaf (`12x6-SLIDE` / `24x6-SLIDE`). The terminal post kit does not emit on a gate-only section. Known gaps `4x6`, `12x8` emit free-text. |
