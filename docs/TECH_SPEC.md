...
| `src/lib/job-form.ts`, `src/lib/job-constants.ts` | Validation; `JOB_TYPES` (`Install`, `Pickup`, `Drop`, `Other`, `Site Walk`, `Relocate`); `JOB_STATUSES` (`Active`, `Cancelled`); `CANCEL_JOB_CONFIRM`; `JOB_CLASSES` vs `SITE_WALK_CLASSES` |
...
**Relocate** (additive, ALE-36): same class dropdown as Install (`EVENT` / `CONSTRUCTION` / `OTHER` — not Non Pay / Site Visit). Create default class is the shared form default (same as Install). Fence, materials, and Generate BOM may be empty via `allowsEmptyFenceAndMaterials` (same placeholder-row skip as Site Walk). `allowsZeroHourLabor` stays Site Walk only, so Relocate labor hours and revenue roll into P&L and a 0/0 labor row is dropped. Inventory sign is 0. Jobs / same-day list visibility matches other types (no revenue or materials predicate).
