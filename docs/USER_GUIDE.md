...
**Gates:** Gate + Gate 2 type/qty. Swing hardware follows Excel. **6′ slides** (`12x6` / `15x6` / `20x6` / `24x6 SLIDE`) use the ALE-30 field recipe (per-size brackets, extra track posts, gate-frame 1-3/8″ pipe). Track terminals (gate ×2 + extras) show when that section has **fence LF > 0** (chainlink, or a panel/barricade host — CL6 `8' x 2-1/2` on the panel/barricade host). Qty 1 with LF > 0: **4 / 5 / 6 / 7** terminals for 12 / 15 / 20 / 24. A **gate-only** section (fence type `-`, LF empty) or chainlink with **LF = 0** does **not** add those posts, tension bars, or bands — you get the gate leaf plus slide hardware (carrier, rollers, brackets, gate-frame top rail). Chainlink with LF > 0 does **not** double them (`24x6` stays **7**, not 14). **8′ slides** stay on Excel-thin math until takeoff — see [BOM_APPROVED.md](./BOM_APPROVED.md) §7. `12x6 SLIDE` and `24x6 SLIDE` are catalog leaves (Match **catalog** when the yard has the SKU). Known catalog gaps `4x6`, `12x8`, `CUSTOM` still emit a **free-text** line and warn; they do **not** create inventory SKUs. `4x8` is not in the dropdown.
